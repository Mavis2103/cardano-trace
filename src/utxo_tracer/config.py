...
Supported env vars:
  ...
  MAESTRO_API_KEY
  MAESTRO_BASE_URL
  MINIBF_API_KEY                Dolos minibf (Blockfrost-compatible REST)
  MINIBF_AUTH_TYPE              project_id | bearer | dmtr-api-key
  MINIBF_BASE_URL
  KUPO_URL
  KUPO_API_KEY
  OGMIOS_URL
...
# Kept in sync with the vars read by _env_overlay() below, so the
# shell-overrides-.env warning covers every provider/CEX var we honour.
_ENV_OVERRIDE_VARS = [
    "UTXO_TRACER_PROVIDER",
    "UTXORPC_API_KEY", "UTXORPC_BASE_URL", "UTXORPC_ENDPOINT_URL",
    "BLOCKFROST_API_KEY", "BLOCKFROST_AUTH_TYPE", "BLOCKFROST_BASE_URL",
    "BLOCKFROST_ENDPOINT_URL",
...
def _env_overlay(base: dict[str, Any]) -> dict[str, Any]:
    ...
    _set(["providers", "minibf", "api_key"], os.getenv("MINIBF_API_KEY"))
    _set(["providers", "minibf", "auth_type"], os.getenv("MINIBF_AUTH_TYPE"))
    _set(["providers", "minibf", "base_url"], os.getenv("MINIBF_BASE_URL"))
...
def set_provider_config(
    ...
    # Write only the fields the caller actually supplied (non-None). Fields
    # that don't apply to a provider (e.g. kupo_url for utxorpc) simply stay
    # None and are skipped, so one code path serves every provider.
    p = cfg["providers"].setdefault(provider, {})
    updates = {
        "api_key": api_key,
        "base_url": base_url,
        "auth_type": auth_type,
